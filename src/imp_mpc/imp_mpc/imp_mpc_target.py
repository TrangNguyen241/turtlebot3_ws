...
#### define variable ####
        ...
        self.timer = self.create_timer(self.Ts, self.imp_mpc_target)



#### MPC Controller ####
    def imp_mpc_target(self):
        ...
        if not(self.target_tracking_finish):
            ...
            y_0 = self.y
            theta_0 = self.theta
            pose_0 = np.array([[x_0 + self.b*np.cos(theta_0)],
                                [y_0 + self.b*np.sin(theta_0)]])  
            self.x_plot.append(pose_0[0, 0])
            self.y_plot.append(pose_0[1, 0]) 
            # Publishing positions of robot to Rviz
            self.publish_actual_path(pose_0[0, 0], pose_0[1, 0])
            error_to_target = np.linalg.norm(pose_0 - self.target)
            ...
            if (error_to_target > self.std_error):
                ...
                objective = 0
                for i in range(0,Npred):
                    objective = objective + cas.mtimes(cas.mtimes(cas.transpose(x[:,i] - self.target), self.Q), x[:,i] - self.target) +\
                                        cas.mtimes(cas.mtimes(cas.transpose(u[:,i]), self.R), u[:,i])
                objective = objective + cas.mtimes(cas.mtimes(cas.transpose(x[:, Npred] - self.target), self.P), x[:, Npred] - self.target)
                ...
                # Solve the problem
                solver_2.set_value(xinit, pose_0)

                sol = solver_2.solve()
                usol = sol.value(u)
                self.u0 = usol[:,0]
                print(self.u0.shape)
                self.u_1_plot.append(self.u0[0])
                self.u_2_plot.append(self.u0[1])
                self.u_real = np.array([[np.cos(theta_0), np.sin(theta_0)],
                                        [-np.sin(theta_0)/self.b, np.cos(theta_0)/self.b]]) @ self.u0
                # Publishing control signal
                cmd_lin_vel = self.u_real[0]
                cmd_ang_vel = self.u_real[1]
                ...
                self.pub_vel(self.u_real[0],self.u_real[1])
                self.lin_v_plot.append(self.u_real[0])
                self.ang_v_plot.append(self.u_real[1])
                self.counter +=1
                time_series = self.counter *self.Ts
                ...
                scipy.io.savemat('/home/nguyehtt/turtlebot3_ws/src/imp_mpc/imp_mpc/imp_target_pos.mat', {'x': self.x_plot, 'y': self.y_plot})
                scipy.io.savemat('/home/nguyehtt/turtlebot3_ws/src/imp_mpc/imp_mpc/imp_target_lin_vel.mat', {'x': self.time, 'y': self.lin_v_plot})
                scipy.io.savemat('/home/nguyehtt/turtlebot3_ws/src/imp_mpc/imp_mpc/imp_target_ang_vel.mat', {'x': self.time, 'y': self.ang_v_plot})
                print("Data saving")
