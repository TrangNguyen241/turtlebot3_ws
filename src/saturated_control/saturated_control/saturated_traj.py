...
class MySingleSaturated(Node):
    def __init__(self):
        ...
        self.Ts = 0.1 #trajectory tracking default: 0.1
        # COntinuous time
        A = np.zeros((2,2))
        self.B = np.eye(2)
        # Discrete time
        ...
        self.gamma = 3 #target tracking # default: 1
        ...
        self.P = np.array([[0.0925, 0],[0, 0.0925]])  # for alpha = 0.0001
        ...
        # Target point
        # self.target_point = np.array([[-5],[2]])
        self.target_point = np.array([[0.5],[1]])
        ...
        self.timer = self.create_timer(self.Ts, self.saturated_control_trajectory)
    ...
    def saturated_control_trajectory(self):
        ...
        if not(self.trajectory_tracking_finish):
            if self.iref < self.imax:
                ...
                # Publishing actual path to Rviz
                ...
                error_square = error_to_target**2
                self.total_error_tracking += error_square
                self.actual_poses[:, self.iref] = pose_robot.reshape(2,)
                self.get_logger().info("Error_to_target: {}".format(error_to_target))
                # Computing the control signal
                start = time.time()
                u_vir_ref = -self.gamma * self.B.T @ self.P @ (pose_robot - ref_point)
        
                # compute error to print 
                # error = pose_robot - ref_point
                # Print u before sat
                self.get_logger().info("u_vir_before_sat: {}".format(u_vir_ref))

                if (self.A_Ue @ u_vir_ref <= self.b_Ue).all():
                    lamda = 1
                else:
                    lamda_list = self.b_Ue / (self.A_Ue @ u_vir_ref) # checked
                    # self.get_logger().info("lamda_list: {}".format(lamda_list))
                ...
                u_vir_sat = lamda * u_vir_ref + self.u_ref_traj[:, self.iref].reshape(2,1)
                ...
                print("Shape of u_vir_sat: ", u_vir_sat.shape)
                end = time.time()
                ...
                scipy.io.savemat('/home/nguyehtt/turtlebot3_ws/src/saturated_control/saturated_control/sat_traj_pos.mat', {'x': self.x_plot, 'y': self.y_plot})
                scipy.io.savemat('/home/nguyehtt/turtlebot3_ws/src/saturated_control/saturated_control/sat_traj_lin_vel.mat', {'x': self.time, 'y': self.lin_v_plot})
                scipy.io.savemat('/home/nguyehtt/turtlebot3_ws/src/saturated_control/saturated_control/sat_traj_ang_vel.mat', {'x': self.time, 'y': self.ang_v_plot})
                print("Data saving")
